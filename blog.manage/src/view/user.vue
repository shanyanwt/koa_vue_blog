  ...
  <div class="user">
        ...
        <Col span="6">
          ...
          <p>
            <Select v-model="form.status" placeholder="全部">
              <Option v-for="(item, index) in 2" :value="index" :key="index">{{
                index | articleStatus
              }}</Option>
            </Select>
          </p>
    ...
    <div>
      ...
      <Row class="page">
        <Page
          placement="top"
          @on-page-size-change="changes"
          @on-change="change"
          :page-size="mypage.pageSN"
          :current="mypage.pageCurrent"
          :total="mypage.total_row_number"
          :page-size-opts="mypage.row_count"
          show-elevator
          show-sizer
        ></Page>
        <span
          >共{{ mypage.total_row_number }}条记录，当前页{{
            (mypage.pageCurrent - 1) * mypage.pageSN + 1
          }}-{{
            mypage.pageCurrent * mypage.pageSN > mypage.total_row_number
              ? mypage.total_row_number
              : mypage.pageCurrent * mypage.pageSN
          }}条</span
        >
      </Row>
    </div>
    ...
    <Modal v-model="modal2" width="360" class-name="vertical-center-modal">
      ...
      <div slot="footer" class="modal-foot">
        <Button type="error" size="large" @click="remove(dataIdenx, '2')"
          >确认</Button
        >
      ...
      columns7: [
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            const text =
              row.status == 0 ? "允许" : row.status == 1 ? "以禁止" : "--";
            return h(
              "Button",
              {
                props: {
  ...
  methods: {
    ...
    add(name) {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          let user_ticket = utils.md5(
            utils
              .md5(
                this.formInline.name.toLowerCase() +
                  utils.md5(this.formInline.password)
              )
              .toLowerCase()
          );
          axios({
            ...
            .then((res) => {
              if (res.error_code == CONSTS.ERROR_CODE.SUCCESS) {
                ...
                res.error_code == CONSTS.ERROR_CODE.CHECK_ALREADY_EXISTS
              ) {
                that.$Notice.error({
                  title: "该创建的用户名已存在！",
        ...
        .then((res) => {
          if (res.error_code == CONSTS.ERROR_CODE.SUCCESS) {
            res.result_data.map((item) => {
              ...
              item.update_time =
                dateFormat.diffTime(item.update_time * 1000) || "--";
            });
    ...
    getAreicle(id) {
      axios({
        ...
        .then((res) => {
          if (res.error_code == CONSTS.ERROR_CODE.SUCCESS) {
            res.result_data.create_time = dateFormat.dateFormat(
              res.result_data.create_time * 1000
            );
